...
function WorkspaceHeader() {
  const { workspaceName, updateWorkspaceName } = useStore();
  const [menuOpen, setMenuOpen] = useState(false);
  ...
  return (
    <>
      ...
      {/* Workspace name + options — aligned with sidebar content rows */}
      <div className="relative mx-2 mb-1 flex items-center gap-2 px-2 h-11 rounded-md min-w-0">
        <AppIcon size={18} className="rounded-[4px] ring-1 ring-[var(--border)]" />
        ...
        {!editing && (
          <div className="relative flex-shrink-0" ref={menuRef}>
            <button
              type="button"
              data-tauri-drag-region="false"
              className="w-6 h-6 flex items-center justify-center rounded text-[var(--text-secondary)] hover:text-[var(--text)] hover:bg-[var(--bg-hover)]"
